...
//timer logic start
const dateLine="2023-09-01"
function getTimeRemaining(end){
const total =Date.parse(end)-Date.parse(new Date())
const days=Math.floor(total / (1000 *60*60*24));/* get the days quantity*/
const hours=Math.floor((total / (1000 * 60 * 60) % 24));/*get the hours quantity*/
...
function setClock(selector,ending){
    ...
    function updateClock(){
        ...
        hoursBlock.textContent=time.hours >=0 && time.hours <10? `0${time.hours}`: time.hours;
        minutesBlock.textContent=time.min >=0 && time.minutes <10? `0${time.min}`:time.min;
